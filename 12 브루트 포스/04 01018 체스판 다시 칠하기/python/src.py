...
for i in range(m):
    ...
    line = input()
    assert len(line) == n
    assert not False in [a in 'BW' for a in line]

    # WB로 시작하면 WB 마스크, BW로 시작하면 BW 마스크 적용.
    if i % 2 == 0:
        masked_line = tuple(line[j] == wb_mask[j] for j in range(n))
    else:
        masked_line = tuple(line[j] != wb_mask[j] for j in range(n))

    # WB로 시작하는 보드 기준 수정 횟수 기록.
    fixed_line = tuple(masked_line[x:x+8].count(False) for x in range(n-7))
